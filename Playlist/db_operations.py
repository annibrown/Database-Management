...
class db_operations():
    ...
    # inserts new songs into the database when there are already songs there
    def insert_new_songs(self, filepath):
        data = helper.data_cleaner(filepath)
        attribute_count = len(data[0])
        data_sorted = []
        for i in range(len(data)):
            new = data[i][0]
            query = "SELECT COUNT(*) FROM songs WHERE songID = \""+new+"\""
            result = self.single_record(query)
            if result == 0:
                data_sorted.append(data[i])
                print("Song successfully entered!")
            else:
                print("A song was already in database, and didn't get inserted again\n")
        placeholders = ("?,"*attribute_count)[:-1]
        query = "INSERT INTO songs VALUES("+placeholders+")"
        self.bulk_insert(query, data_sorted)
